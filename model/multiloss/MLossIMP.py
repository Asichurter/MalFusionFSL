...
class MLossIMP(BaseMultiLossModel):
    ...
    def _feature_forward(self, support_features, query_features,
        ...
        support_features = support_features.view(n * k, -1).unsqueeze(0)
        query_features = query_features.view(qk, -1).unsqueeze(0)
        ...
        protos = self._compute_protos(support_features, prob_support)
        ...
        for ii in range(self.NumClusterSteps):
            ...
            for i, ex in enumerate(support_features[0]):
            ...
            if nClusters > nInitialClusters:
                ...
                prob_support = assign_cluster_radii_limited(protos, support_features, radii,
        ...
        # 计算query的类簇logits
        logits = compute_logits_radii(protos, query_features, radii, use_sigma=self.Sigma is not None).squeeze()
